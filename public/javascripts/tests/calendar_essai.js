	 ...
	 app.directive("fillReservTable", ["$compile","$http",  function ($compile, $http) {    
	    ...
	    return {
	        ...
	        link: function (scope, element, attr) {
	            ...
	            $http.get('http://localhost:3000/testcalendar/rooms_list').success(function(roomlist){
					scope.rooms_list = roomlist;
					console.log(roomlist);
					var html="";
					for (x=0; x<roomlist.length ; x++){
						room = roomlist[x];
						console.log(room)
			            ADRESS = 'http://localhost:3000/testcalendar/reservations/'+room._id.toString() + '/' + beginCal + '/' + endCal
			            html += "<tr><th>" + room.room_name + "</th>"
			            $http.get(ADRESS).success(function(data){
			            	...
			            	if (data.length == 0) {  // if there is no reservation for this room on this period
								while (i <= scope.days.length){
									...
									html+="<td class='emptycell' colspan="+colspan+" ng-click='cal.showResForm(); cal.selectday("+i+", \""+ room._id.toString() +"\");'></td>";
